...
public class LayarUtama extends JFrame {
	...
	public LayarUtama() {
		...
		JLabel lblTitulo = new JLabel("Konverter Mata Uang");
		...
		JLabel lblConverterPara = new JLabel("ubah ke");
		...
		JButton btnConverter = new JButton("Untuk mengubah");
		btnConverter.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					JOptionPane.showMessageDialog(null, "Bidang \"Nilai untuk dikonversi\" harus berupa angka desimal.", "Kesalahan!", 2);
				} catch (JsonSyntaxException error) {
					JOptionPane.showMessageDialog(null, "Nilai yang akan dikonversi terlalu luas.", "Kesalahan!", 2);
				}
			}
		});
		btnConverter.setBounds(155, 103, 130, 21);
		contentPane.add(btnConverter);
		
		JLabel lblValor = new JLabel("Nilai untuk dikonversi:");
		lblValor.setHorizontalAlignment(SwingConstants.RIGHT);
		lblValor.setFont(new Font("Tahoma", Font.PLAIN, 12));
		lblValor.setBounds(10, 80, 205, 13);
		contentPane.add(lblValor);
		
		JLabel lblTbCotacao = new JLabel("Kutipan mata uang berdasarkan dolar AS");
		...
		JButton btnVoltar = new JButton("Kembali");
